...
tf.app.flags.DEFINE_string("data_dir", "I:/seq2seq-master/data/", "Data directory")
tf.app.flags.DEFINE_string("train_dir", "I:/seq2seq-master/data/", "Training directory.")
...
def decode():
  with tf.Session() as sess:
    ...
    # 加载词汇表
    en_vocab_path = os.path.join(FLAGS.data_dir,"engvoca.utf8")    # edit by Ken
    fr_vocab_path = os.path.join(FLAGS.data_dir,"chivoca.utf8")   # % FLAGS.fr_vocab_size)
    ...
    while sentence:
      # 先把输入的单词,转换成索引形式
      token_ids = data_utils.sentence_to_token_ids(tf.compat.as_bytes(sentence), en_vocab, False)  # True->chi False->eng edited by Ken
